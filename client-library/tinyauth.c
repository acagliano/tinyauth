...
void tinyauth_open(struct tinyauth_key* keyobj, const char *fname, const char *passphrase){
  ...
  if(!file) {keyobj->error = TA_IO_ERROR; return;}
  ...
  if(cryptx_asn1_decode(unwrap1, unwrap1_len, 0, NULL, NULL, &encrypted)){
    keyobj->error = TA_DECODE_ERROR; return;}
  
  keyobj->encrypted = *((uint8_t*)encrypted);
  if(keyobj->encrypted){
    // if key encrypted
    if(passphrase == NULL) {keyobj->error = TA_PASSPHRASE_REQD; return;}
    ...
    if(cryptx_aes_init(&aes, nonces, CRYPTX_AES_256_KEYLEN, &nonces[CRYPTX_AES_256_KEYLEN], CRYPTX_AES_IV_SIZE, CRYPTX_AES_GCM_FLAGS)){
      keyobj->error = TA_DECRYPTION_ERROR; return;
    }
    
    // verify tag
    if(!cryptx_aes_verify(&aes, NULL, 0, keyobj->credentials, keyobj->credentials_len, tag)) {
      keyobj->error = TA_INTEGRITY_ERROR; return;
    }
...
size_t tinyauth_serialize_for_transfer(struct tinyauth_key* keyobj, uint8_t* packet, uint8_t serialization_mode){
  ...
  if(cryptx_asn1_decode(keyobj->credentials, keyobj->credentials_len, 0, NULL, &username_len, &username)){
    keyobj->error = TA_DECODE_ERROR; return 0;}
  if(cryptx_asn1_decode(keyobj->credentials, keyobj->credentials_len, 1, NULL, &token_len, &token)){
    keyobj->error = TA_DECODE_ERROR; return 0;}
  
  switch(serialization_mode){
    case TA_SERIALIZE_0TERM:
      memcpy(packet, username, username_len);
      packet[username_len] = 0;
      memcpy(&packet[username_len+1], token, token_len);
      return username_len + 1 + token_len;
      break;
    case TA_SERIALIZE_LPREPEND:
      *((size_t*)packet) = username_len;
      memcpy(packet+3, username, username_len);
      *((size_t*)packet+3+username_len) = token_len;
      memcpy(packet+3+username_len+3, token, token_len);
      return username_len + token_len + 6;
      break;
    default:
      keyobj->error = TA_SERIALIZATION_ERROR;
      return 0;
  }
}
